...
class _WomenScreenState extends State<WomenScreen> with TickerProviderStateMixin {
  ...
  final List<Map<String, dynamic>> _categories = [
    {'name': 'All', 'icon': Icons.grid_view, 'gradient': [const Color(0xFFF06292), const Color(0xFFF8BBD0)]},
    {'name': 'Dresses', 'icon': Icons.woman, 'gradient': [const Color(0xFFBA68C8), const Color(0xFFE1BEE7)]},
    {'name': 'Tops', 'icon': Icons.checkroom, 'gradient': [const Color(0xFF4DB6AC), const Color(0xFFB2DFDB)]},
    {'name': 'Skirts', 'icon': Icons.layers, 'gradient': [const Color(0xFFFFB74D), const Color(0xFFFFE0B2)]},
    {'name': 'Heels', 'icon': Icons.do_not_step, 'gradient': [const Color(0xFFE57373), const Color(0xFFFFCDD2)]},
    {'name': 'Bags', 'icon': Icons.shopping_bag, 'gradient': [const Color(0xFF7986CB), const Color(0xFFC5CAE9)]},
    {'name': 'Jewelry', 'icon': Icons.diamond, 'gradient': [const Color(0xFFFFD54F), const Color(0xFFFFF9C4)]},
  ];
  ...
  final List<Map<String, String>> _banners = [
    ...
    {
      'image': 'https://images.unsplash.com/photo-1534126511673-b6899657816a?w=1200',
      'title': 'Designer Bags',
  ...
  void _initializeProducts() {
    List<Map<String, dynamic>> baseProducts = [
      {'name': 'Floral Summer Dress', 'brand': 'Zara', 'price': 59.99, 'originalPrice': 79.99, 'rating': 4.7, 'reviews': 120, 'image': 'https://images.unsplash.com/photo-1572804013309-59a88b7e92f1?w=500', 'tag': 'Bestseller', 'category': 'Dresses'},
      {'name': 'Silk Blouse', 'brand': 'H&M', 'price': 39.99, 'rating': 4.5, 'reviews': 85, 'image': 'https://images.unsplash.com/photo-1564257631407-4deb1f99d992?w=500', 'tag': 'New', 'category': 'Tops'},
      {'name': 'Leather Handbag', 'brand': 'Gucci', 'price': 499.99, 'originalPrice': 650.00, 'rating': 4.9, 'reviews': 320, 'image': 'https://images.unsplash.com/photo-1584917865442-de89df76afd3?w=500', 'tag': 'Luxury', 'category': 'Bags'},
      {'name': 'Classic Stilettos', 'brand': 'Jimmy Choo', 'price': 250.00, 'rating': 4.8, 'reviews': 150, 'image': 'https://images.unsplash.com/photo-1543163521-1bf539c55dd2?w=500', 'tag': 'Hot', 'category': 'Heels'},
      {'name': 'Gold Necklace', 'brand': 'Tiffany', 'price': 150.00, 'rating': 4.9, 'reviews': 200, 'image': 'https://images.unsplash.com/photo-1599643478518-17488fbbcd75?w=500', 'tag': null, 'category': 'Jewelry'},
    ];
  ...
  List<Map<String, dynamic>> _generateMoreProducts() {
    ...
    final brands = ['Zara', 'H&M', 'Gucci', 'Prada', 'Chanel', 'Forever 21', 'Mango'];
    final categories = ['Dresses', 'Tops', 'Skirts', 'Heels', 'Bags', 'Jewelry'];
    final images = [
      'https://images.unsplash.com/photo-1595777457583-95e059d581b8?w=500', // Dress
      'https://images.unsplash.com/photo-1551163943-3f6a29e39c66?w=500', // Top
      'https://images.unsplash.com/photo-1583496661160-fb5886a0aaaa?w=500', // Skirt
      'https://images.unsplash.com/photo-1566174053879-31528523f8ae?w=500', // Dress 2
      'https://images.unsplash.com/photo-1589125140701-70287329d41e?w=500', // Bag
    ];

    for (int i = 0; i < 50; i++) {
      var category = categories[i % categories.length];
      var brand = brands[i % brands.length];
      var price = (math.Random().nextInt(200) + 20).toDouble();
      ...
      generated.add({
        ...
        'rating': 4.0 + (math.Random().nextDouble()),
          ...
          return Container(
            ...
            child: Column(
              children: [
                ...
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text("Filter & Sort", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), TextButton(onPressed: () { setState(() { _sortOption = 'default'; _selectedBrands.clear(); }); setStateModal(() {}); }, child: const Text("Reset All", style: TextStyle(color: Colors.pink)))]),
                const Divider(),
                Expanded(
  ...
  Widget _buildChoiceChip(String label, bool isSelected, Function(bool) onSelected) {
    return ChoiceChip(label: Text(label), selected: isSelected, onSelected: onSelected, selectedColor: Colors.pink.shade100, backgroundColor: Colors.grey.shade100, labelStyle: TextStyle(color: isSelected ? Colors.pink.shade900 : Colors.black));
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xFFFFF0F5), // Lavender Blush (Pinkish)
  ...
  Widget _buildAppBar(BuildContext context) {
    return SliverAppBar(
      ...
      title: Row(children: [Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(gradient: LinearGradient(colors: [Colors.pink.shade400, Colors.purple.shade400]), borderRadius: BorderRadius.circular(10)), child: const Text('👗', style: TextStyle(fontSize: 20))), const SizedBox(width: 12), const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("Women's Fashion", style: TextStyle(color: Colors.black, fontSize: 18, fontWeight: FontWeight.w900, letterSpacing: -0.5)), Text("Elegant Collection", style: TextStyle(color: Colors.grey, fontSize: 10, fontWeight: FontWeight.w500))])]),
  ...
  // Reuse _buildSearchBar, _buildCarouselBanner, _buildCategorySelector, _buildSectionTitle, _buildProductGrid from MenScreen
  // BUT UPDATE COLORS TO PINK THEME inside them (omitted for brevity, assume structure matches but with pink accents)
  // For the purpose of the complete code, I will implement the critical parts below with the pink theme.
